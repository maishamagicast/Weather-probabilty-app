    ...
    if (super._changed(), this._clear) {
      ...
      if (e2 > 0 && i > 0) {
        ...
        if (d.moveTo(s, 0), p2 > 0 && u2 > 0) {
          let i2 = Math.round(pe(o, s + r, e2 - r - s));
          l = pe(l, -1 / 0, 0), d.lineTo(i2 - r, 0), d.lineTo(o, l), d.lineTo(i2 + r, 0);
        }
        if (d.lineTo(e2 - s, 0), d.arcTo(e2, 0, e2, s, s), p2 > 0 && u2 < 0) {
          let a2 = Math.round(pe(l, s + r, i - r - s));
          o = pe(o, e2, 1 / 0), d.lineTo(e2, s), d.lineTo(e2, Math.max(a2 - r, s)), d.lineTo(o, l), d.lineTo(e2, a2 + r);
        }
        if (d.lineTo(e2, i - s), d.arcTo(e2, i, e2 - s, i, s), p2 < 0 && u2 < 0) {
          let a2 = Math.round(pe(o, s + r, e2 - r - s));
          l = pe(l, i, 1 / 0), d.lineTo(e2 - s, i), d.lineTo(a2 + r, i), d.lineTo(o, l), d.lineTo(a2 - r, i);
        }
        if (d.lineTo(s, i), d.arcTo(0, i, 0, i - s, s), p2 < 0 && u2 > 0) {
          let e3 = Math.round(pe(l, s + r, i - s - r));
          o = pe(o, -1 / 0, 0), d.lineTo(0, i - s), d.lineTo(0, e3 + r), d.lineTo(o, l), d.lineTo(0, Math.max(e3 - r, s));
        }
...
var u = class extends ai {
  ...
  _afterNew() {
    ...
    this._setDefaultFn("background", (() => p.new(this._root, {}))).set("themeTags", ["tooltip", "background"]), this._label = this.children.push(oi.new(this._root, {})), this._disposers.push(this._label.events.on("boundschanged", (() => {
      ...
      e2 && (this._disposers.push(J(e2, "pointerover", ((e3) => {
        this._htmlContentHovered = true;
      }))), this._disposers.push(J(e2, "pointerout", ((e3) => {
  ...
  _dispose() {
    super._dispose(), k(this._root._tooltips, this);
  }
  _updateChildren() {
  ...
  _changed() {
    if (super._changed(), (this.isDirty("pointTo") || this.isDirty("pointerOrientation")) && this._updateBackground(), this.isDirty("tooltipTarget") && this.updateBackgroundColor(), this.isDirty("keepTargetHover")) {
      if (this.get("keepTargetHover")) {
        ...
        this._keepHoverDp = new X([e2.events.on("pointerover", ((e3) => {
          ...
          this._keepHoverDp && t && this._keepHoverDp.disposers.push(J(t, "pointerleave", ((t2) => {
            const i = this.root._renderer.getEvent(t2);
            e2.events.dispatch("pointerout", { type: "pointerout", originalEvent: i.event, point: i.point, simulated: false, target: e2 });
          })));
        }));
  ...
  _updateTextColor(e2) {
    this.get("autoTextColor") && (null == e2 && (e2 = this.get("background").get("fill")), null == e2 && (e2 = this._root.interfaceColors.get("background")), e2 instanceof Yt && this.label.set("fill", Yt.alternative(e2, this._root.interfaceColors.get("alternativeText"), this._root.interfaceColors.get("text"))));
  }
  _setDataItem(e2) {
  ...
  _updateBackground() {
    ...
    if (e2) {
      ...
      const g = this.get("bounds", { left: -u2, top: -d, right: r - u2, bottom: a - d });
      this._updateBounds();
      let b = this.width(), c = this.height();
      0 === b && (b = this._w), 0 === c && (c = this._h);
      let _ = this.get("pointTo", { x: r / 2, y: a / 2 }), f = _.x, m = _.y, v = this.get("pointerOrientation"), T = this.get("background"), y = 0, D = 0, k2 = 0;
      T instanceof p && (y = T.get("pointerLength", 0), D = T.get("strokeWidth", 0) / 2, k2 = D, T.set("width", b), T.set("height", c));
      let w = 0, x = 0, P = g.right - g.left, C = g.bottom - g.top;
      "horizontal" == v || "left" == v || "right" == v ? (D = 0, "horizontal" == v ? f > g.left + P / 2 ? (f -= b * (1 - i) + y, k2 *= -1) : f += b * i + y : "left" == v ? f += b * (1 - i) + y : (f -= b * i + y, k2 *= -1)) : (k2 = 0, "vertical" == v ? m > g.top + c / 2 + y ? m -= c * (1 - s) + y : (m += c * s + y, D *= -1) : "down" == v ? m -= c * (1 - s) + y : (m += c * s + y, D *= -1)), f = pe(f, g.left + b * i, g.left + P - b * (1 - i)) + k2, m = pe(m, g.top + c * s, g.top + C - c * (1 - s)) - D, w = _.x - f + b * i + k2, x = _.y - m + c * s - D, this._fx = f, this._fy = m;
      const O = this.get("animationDuration", 0);
      if (O > 0 && this.get("visible") && this.get("opacity") > 0.1) {
        const e3 = this.get("animationEasing");
        this.animate({ key: "x", to: f, duration: O, easing: e3 }), this.animate({ key: "y", to: m, duration: O, easing: e3 });
      } else this.set("x", f), this.set("y", m);
      T instanceof p && (T.set("pointerX", w), T.set("pointerY", x)), b > 0 && (this._w = b), c > 0 && (this._h = c);
    }
  }
};
Object.defineProperty(u, "className", { enumerable: true, configurable: true, writable: true, value: "Tooltip" }), Object.defineProperty(u, "classNames", { enumerable: true, configurable: true, writable: true, value: ai.classNames.concat([u.className]) });

export {
  u
};
//# sourceMappingURL=chunk-YKAHWUTP.js.map
