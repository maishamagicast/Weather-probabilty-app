...
var T = class {
  async queryAll(e2, i2, n2) {
    ...
    const s2 = { ...R2, ...n2, returnSampleInfo: true }, a2 = await this.query(e2[e2.length - 1], o2, s2), r = await this._queryAllContinue(e2, a2, s2);
    return r.geometry = r.geometry.export(), l3 && delete r.sampleInfo, r;
  }
  async query(e2, i2, n2) {
    ...
    const o2 = { ...R2, ...n2 }, l3 = new x2(e2, i2.spatialReference, o2), s2 = o2.signal;
    return await e2.load({ signal: s2 }), await q(l3, i2, s2), await this._selectTiles(l3, s2), await F(l3, s2), b(l3), z(l3, s2);
  }
  async createSampler(e2, i2, n2) {
    if (!e2) throw new s("elevation-query:invalid-layer", "Elevation queries require an elevation layer to fetch tiles from");
  ...
  async createSamplerAll(e2, i2, n2) {
    if (!(e2 = n2?.ignoreInvisibleLayers ? e2.filter(((e3) => e3.visible)) : e2.slice()).length) throw new s("elevation-query:invalid-layer", "Elevation queries require at least one elevation layer to fetch tiles from");
    if (!i2 || "extent" !== i2.type) throw new s("elevation-query:invalid-extent", "Invalid or undefined extent");
    const o2 = { ...R2, ...n2, returnSampleInfo: true }, l3 = await this._createSampler(e2[e2.length - 1], i2, o2);
    return this._createSamplerAllContinue(e2, i2, l3, o2);
  }
  async _createSampler(e2, t4, i2, n2) {
    const o2 = i2.signal;
    await e2.load({ signal: o2 });
    const l3 = t4.spatialReference, s2 = e2.tileInfo.spatialReference;
    l3.equals(s2) || (await Q([{ source: l3, dest: s2 }], { signal: o2 }), t4 = O(t4, s2));
    const a2 = new _3(e2, t4, i2, n2);
    return await this._selectTiles(a2, o2), await F(a2, o2), new u2(a2.elevationTiles, a2.layer.tileInfo, a2.options.noDataValue);
  }
  async _createSamplerAllContinue(e2, t4, i2, n2) {
    if (e2.pop(), !e2.length) return i2;
    const o2 = i2.samplers.filter(((e3) => !e3.tile.hasNoDataValues)).map(((e3) => o(e3.extent))), l3 = await this._createSampler(e2[e2.length - 1], t4, n2, o2);
  ...
  async _queryAllContinue(e2, t4, n2) {
    ...
    const c3 = t4.geometry.clone(a2), u3 = await this.query(e2[e2.length - 1], c3, n2), h3 = u3.sampleInfo;
  ...
  async _selectTiles(e2, i2) {
    ...
    else if ("finest-contiguous" === n2) await this._selectTilesFinestContiguous(e2, i2);
    else {
  ...
  async _selectTilesFinestContiguousAt(e2, i2, l3) {
    const s2 = e2.layer;
    ...
    try {
      ...
      else if (await F(e2, l3), !e2.allElevationTilesFetched()) throw e2.clearElevationTiles(), new s("elevation-query:has-unavailable-tiles", "Some elevation tiles are unavailable");
    } catch (c3) {
      a(c3), await this._selectTilesFinestContiguousAt(e2, i2 - 1, l3);
    }
  }
  async _selectTilesAuto(t4, i2) {
    ...
    const s2 = t4.getTilesToFetch(), a2 = {}, r = s2.map((async (t5) => {
      const n2 = new t(null, 0, 0, 0, u()), s3 = await _(l3.fetchAvailabilityUpsample(t5.level, t5.row, t5.col, n2, { signal: i2 }));
      false !== s3.ok ? null != t5.id && (a2[t5.id] = n2) : a(s3.error);
    }));
    await h(Promise.all(r), i2), t4.remapTiles(a2);
  }
...
var v = class _v {
  ...
  async project(e2, t4) {
    ...
    await Q([{ source: this.spatialReference, dest: e2 }], { signal: t4 });
...
async function q(e2, i2, n2) {
  let o2;
  const l3 = e2.layer.tileInfo.spatialReference;
  if (i2 instanceof v ? o2 = await i2.project(l3, n2) : (await Q([{ source: i2.spatialReference, dest: l3 }], { signal: n2 }), o2 = O(i2, l3)), !o2) throw new s("elevation-query:spatial-reference-mismatch", `Cannot query elevation in '${i2.spatialReference.wkid}' on an elevation service in '${l3.wkid}'`);
...
async function F(e2, t4) {
  const i2 = e2.getTilesToFetch(), o2 = {}, l3 = e2.options.cache, s2 = e2.options.noDataValue, a2 = i2.map((async (i3) => {
    if (null == i3.id) return;
    const n2 = `${e2.layer.uid}:${i3.id}:${s2}`, a3 = null != l3 ? l3.get(n2) : null, r = null != a3 ? a3 : await e2.layer.fetchTile(i3.level, i3.row, i3.col, { noDataValue: s2, signal: t4 });
...
//# sourceMappingURL=ElevationQuery-5IZ272JQ.js.map
