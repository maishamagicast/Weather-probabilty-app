import {
  S
} from "./chunk-25VP2OQW.js";
import {
  i as i3
} from "./chunk-YKZMZKSQ.js";
import {
  H as H2
} from "./chunk-4MVMXF4W.js";
import {
  v
} from "./chunk-MEDFBA6E.js";
import {
  A as A3
} from "./chunk-V62WAS25.js";
import {
  A as A2,
  I
} from "./chunk-NSUMVOT3.js";
import {
  c,
  i as i4,
  o as o2,
  t as t2
} from "./chunk-XOOEFPJ4.js";
import {
  h
...
async function m2(e2, t3, r3) {
  const { geometry: s5 } = t3, i5 = { ...t3.attributes };
...
async function f3(e2, t3) {
  const r3 = await Promise.all((t3.addAttachments ?? []).map(((t4) => g2(e2, t4)))), a3 = await Promise.all((t3.updateAttachments ?? []).map(((t4) => g2(e2, t4)))), s5 = t3.deleteAttachments ?? [];
...
function h2(t3, r3) {
  return new d({ attributes: t3.attributes, geometry: f2({ ...t3.geometry, spatialReference: r3 }) });
}
function I2(e2, t3) {
...
var G2 = class extends p {
  ...
  load(e2) {
    const t3 = this.layer.sourceJSON, s5 = this._fetchService(t3, { ...e2 }).then((() => this.layer.setUserPrivileges(this.sourceJSON.serviceItemId, e2))).then((() => this._ensureLatestMetadata(e2)));
  ...
  async applyEdits(e2, t3) {
    ...
    const { layer: a3 } = this;
    await H2(a3, "editing");
    const n = "infoFor3D" in a3 ? a3.infoFor3D : null, o3 = null != n, u3 = o3 || (t3?.globalIdUsed ?? false), l2 = o3 ? await this._uploadMeshesAndGetAssetMapEditsJSON(e2) : null, c2 = e2.addFeatures?.map(((e3) => m2(this.layer, e3, n))) ?? [], d2 = (await Promise.all(c2)).filter(L), p2 = e2.updateFeatures?.map(((e3) => m2(this.layer, e3, n))) ?? [], y2 = (await Promise.all(p2)).filter(L), h3 = b(this.layer, e2.deleteFeatures, u3);
    i3(d2, y2, a3.spatialReference);
    const m3 = await f3(this.layer, e2), f4 = a3.capabilities.editing.supportsAsyncApplyEdits && o3, g3 = t3?.gdbVersion || a3.gdbVersion, w = { gdbVersion: g3, rollbackOnFailure: t3?.rollbackOnFailureEnabled, useGlobalIds: u3, returnEditMoment: t3?.returnEditMoment, usePreviousEditMoment: t3?.usePreviousEditMoment, async: f4 };
    ...
    let O;
    try {
      O = f4 ? await this._asyncApplyEdits(E + "/applyEdits", S2) : await H(E + "/applyEdits", S2);
    } catch (_) {
      if (!j(_)) throw _;
      S2.authMode = "immediate", O = f4 ? await this._asyncApplyEdits(E + "/applyEdits", S2) : await H(E + "/applyEdits", S2);
    }
  ...
  async queryFeatures(e2, t3) {
    await this.load();
    const s5 = await this.queryTask.execute(e2, { ...t3, query: this._createRequestQueryOptions(t3) });
  ...
  async uploadAssets(e2, t3) {
    const { uploadAssets: s5 } = await import("./uploadAssets-SQUIIQTL.js");
  ...
  async _uploadMeshesAndGetAssetMapEditsJSON(e2) {
    ...
    for (const i5 of t3) {
      ...
      if (t(t4)) r3.push(e3);
        ...
        const t5 = e3.origin, { convertMeshVertexSpace: s6 } = await import("./convertMeshVertexSpace-FTR36BHB.js"), n = await s6(e3, new i2({ origin: [t5.x, t5.y, t5.z ?? 0] }));
  ...
  _getLayerRequestOptions(e2 = {}) {
    ...
    return { ...e2, query: { gdbVersion: r3, layer: "dynamicDataSource" in t3 && t3.dynamicDataSource ? JSON.stringify({ source: t3.dynamicDataSource }) : void 0, ...s5.query, f: "json", ...this._createRequestQueryOptions(e2) }, responseType: "json" };
  }
  async _areAllAssetsAlreadyMapped(e2) {
    ...
    const { assetMapFieldRoles: { parentGlobalId: c2, assetHash: d2 } } = i5, p2 = new R({ where: `${c2} IN (${l2.map(((e3) => `'${e3}'`))})`, outFields: [d2, c2], returnGeometry: false }), y2 = await s4(o3, p2), { features: m3 } = y2;
...
//# sourceMappingURL=chunk-VAS22NFH.js.map
