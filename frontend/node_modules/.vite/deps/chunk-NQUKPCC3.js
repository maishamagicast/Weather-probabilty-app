import {
  $,
  N
} from "./chunk-HRIEIPLV.js";
import {
  r
} from "./chunk-XRTNDGK2.js";
import {
  o
} from "./chunk-P4O4YCLI.js";
import {
  Te
} from "./chunk-W5UOLFE2.js";
import {
  m
} from "./chunk-MGA7XBRB.js";

// node_modules/@arcgis/core/renderers/support/numberUtils.js
var n = /^-?(\d+)(\.(\d+))?$/i;
function e(t, n2) {
  return t - n2;
}
function r2(t, n2) {
  let e2, r4;
  return e2 = Number(t.toFixed(n2)), e2 < t ? r4 = e2 + 1 / 10 ** n2 : (r4 = e2, e2 -= 1 / 10 ** n2), e2 = Number(e2.toFixed(n2)), r4 = Number(r4.toFixed(n2)), [e2, r4];
}
function o2(t, n2, e2, r4, o4) {
  const i2 = l(t, n2, e2, r4), u3 = null == i2.previous || i2.previous <= o4, s3 = null == i2.next || i2.next <= o4;
  return u3 && s3 || i2.previous + i2.next <= 2 * o4;
}
function i(t) {
  const e2 = String(t), r4 = e2.match(n);
  if (r4?.[1]) return { integer: r4[1].split("").length, fractional: r4[3] ? r4[3].split("").length : 0 };
  if (e2.toLowerCase().includes("e")) {
    const t2 = e2.split("e"), n2 = t2[0], r5 = t2[1];
    if (n2 && r5) {
      const t3 = Number(n2);
      let e3 = Number(r5);
      const o4 = e3 > 0;
      o4 || (e3 = Math.abs(e3));
      const l3 = i(t3);
      return o4 ? (l3.integer += e3, e3 > l3.fractional ? l3.fractional = 0 : l3.fractional -= e3) : (l3.fractional += e3, e3 > l3.integer ? l3.integer = 1 : l3.integer -= e3), l3;
    }
  }
  return { integer: 0, fractional: 0 };
}
function l(t, n2, e2, r4) {
  const o4 = { previous: null, next: null };
  if (null != e2) {
    const r5 = t - e2, i2 = n2 - e2 - r5;
    o4.previous = Math.floor(Math.abs(100 * i2 / r5));
  }
  if (null != r4) {
    const e3 = r4 - t, i2 = r4 - n2 - e3;
    o4.next = Math.floor(Math.abs(100 * i2 / e3));
  }
  return o4;
}
function u(t, n2 = {}) {
  const l3 = t.slice(), { tolerance: u3 = 2, strictBounds: s3 = false, indexes: c3 = l3.map(((t2, n3) => n3)) } = n2;
  c3.sort(e);
  for (let e2 = 0; e2 < c3.length; e2++) {
    const t2 = c3[e2], n3 = l3[t2], a = 0 === t2 ? null : l3[t2 - 1], f2 = t2 === l3.length - 1 ? null : l3[t2 + 1], m3 = i(n3).fractional;
    if (m3) {
      let i2, c4 = 0, g = false;
      for (; c4 <= m3 && !g; ) {
        const t3 = r2(n3, c4);
        i2 = s3 && 0 === e2 ? t3[1] : t3[0], g = o2(n3, i2, a, f2, u3), c4++;
      }
      g && (l3[t2] = i2);
    }
  }
  return l3;
}
var s = { maximumFractionDigits: 20 };
function c(n2) {
  return o(n2, s);
}

// node_modules/@arcgis/core/widgets/Legend/support/utils.js
var o3 = "<";
var l2 = ">";
function r3(e2, A, i2, a) {
  let r4 = "";
  0 === A ? r4 = `${o3} ` : A === i2 && (r4 = `${l2} `);
  return r4 + (a ? N(e2, a) : c(e2));
}
var s2 = ["data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABQAAAAUCAYAAACNiR0NAAAACXBIWXMAAAAAAAAAAAHqZRakAAAANUlEQVQ4jWPMy8v7z0BFwMLAwMAwcdIkqhiWn5fHwEQVk5DAqIGjBo4aOGrgqIEQwEjtKgAATl0Hu6JrzFUAAAAASUVORK5CYII=", "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABQAAAAUCAYAAACNiR0NAAAACXBIWXMAAAAAAAAAAAHqZRakAAAANUlEQVQ4jWPMy8v7z0BFwMLAwMAwaeIkqhiWl5/HwEQVk5DAqIGjBo4aOGrgqIEQwEjtKgAATl0Hu6sKxboAAAAASUVORK5CYII=", "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABQAAAAUCAYAAACNiR0NAAAACXBIWXMAAAAAAAAAAAHqZRakAAAANUlEQVQ4jWPMy8v7z0BFwMLAwMAwadJEqhiWl5fPwEQVk5DAqIGjBo4aOGrgqIEQwEjtKgAATl0Hu75+IUcAAAAASUVORK5CYII="];
async function u2(e2) {
  if (!("visualVariables" in e2) || !e2.visualVariables) return null;
  const A = e2.visualVariables.find(((e3) => "color" === e3.type));
  if (!A) return null;
  let t = null, n2 = null;
  if (A.stops) {
    if (1 === A.stops.length) return A.stops[0].color;
    t = A.stops[0].value, n2 = A.stops[A.stops.length - 1].value;
  }
  const i2 = null != t && null != n2 ? t + (n2 - t) / 2 : 0, { getColor: a } = await import("./visualVariableUtils-IGLTDDNB.js");
  return a(A, i2) ?? null;
}
async function p(e2, A) {
  const t = e2.trailCap, n2 = e2.trailWidth || 1, i2 = A || await u2(e2) || e2.color;
  return new m({ cap: t, color: i2, width: n2 });
}
function f(e2, A) {
  if (!A.field) return null;
  if ("featureReduction" in e2) switch (e2.featureReduction?.type) {
    case "cluster":
    case "binning": {
      const t = e2.featureReduction.fields.find((({ name: e3 }) => e3.toLowerCase() === A.field.toLowerCase()));
      return t && "getField" in e2 ? e2.getField(t.onStatisticField) : null;
    }
  }
  return "getField" in e2 ? e2.getField?.(A.field) : null;
}
function m2(e2, A) {
  const t = "popupTemplate" in e2 ? e2.popupTemplate?.fieldInfos : null;
  if (t?.length && A) return t.find(((e3) => e3.fieldName?.toLowerCase() === A.toLowerCase()))?.format?.dateFormat;
}
function d(t, n2, a) {
  const o4 = f(t, n2);
  if (!o4 || !$(o4) && !Te(o4)) return null;
  let l3 = m2(t, o4.name);
  if (!l3 && "date" === o4.type) {
    let A = 0, t2 = 0;
    n2.stops ? (A = n2.stops?.at(0)?.value ?? A, t2 = n2.stops?.at(-1)?.value ?? t2) : "minDataValue" in n2 && "maxDataValue" in n2 && (A = n2.minDataValue ?? A, t2 = n2.maxDataValue ?? t2), l3 = t2 - A > 2 * r.days ? "short-date" : "short-date-short-time";
  }
  return { fieldType: o4.type, format: l3, timeZoneOptions: { layerTimeZone: "preferredTimeZone" in t ? t.preferredTimeZone : null, viewTimeZone: a, datesInUnknownTimezone: "datesInUnknownTimezone" in t && t.datesInUnknownTimezone } };
}
function c2(e2, A) {
  if ("authoringInfo" in e2) return e2.authoringInfo?.visualVariables?.find((({ theme: e3 }) => e3 === A));
}

export {
  i,
  l,
  u,
  o3 as o,
  l2,
  r3 as r,
  s2 as s,
  u2,
  p,
  d,
  c2 as c
};
//# sourceMappingURL=chunk-NQUKPCC3.js.map
