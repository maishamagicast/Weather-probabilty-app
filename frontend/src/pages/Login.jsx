...
function Login({ onLogin, onNavigateHome, onNavigateSignup }) {
  ...
  const handleDemoLogin = async (demoEmail = 'demo@nasa.earth') => {
    setLoading(true);
    setError('');

    try {
      const result = await authAPI.login(demoEmail, 'demo123');
      if (result.success) {
        onLogin(result.user);
      }
    } catch (err) {
            ...
            <div className="p-8">
              ...
              <div className="mt-8 pt-6 border-t border-cyan-500/20">
                ...
                <div className="space-y-3">
                  <button 
                    onClick={() => handleDemoLogin('farmer.john@agri.com')}
                    disabled={loading}
                  ...
                  >
                    John Kiprop (Demo Farmer)
                  </button>
                  <button 
                    onClick={() => handleDemoLogin('mary.wanjiku@farm.co.ke')}
                    disabled={loading}
                    className="w-full px-4 py-3 bg-white/5 border border-cyan-500/20 rounded-lg text-cyan-300 hover:bg-cyan-500/10 hover:text-cyan-400 transition-all duration-300 disabled:opacity-50"
                  >
                    Mary Wanjiku (Demo Farmer)
                  </button>
                </div>
              </div>
            </div>
